...
@pytest.mark.asyncio
async def test_default_rollout(async_client, test_data):
    device = test_data["device_default"]
    firmware = test_data["firmware_default"]
    ...
    assert (
        ...
        == "http://test/DEFAULT/download/v1/device1/1"
    )
    assert (
